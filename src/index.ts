...
interface SalesRepData {
    [territoryName: string]: SalesRepInfo;
}

// State Abbreviations
const stateAbbreviations: { [stateName: string]: string } = {
...
const salesRepData: SalesRepData = {
...
states.features.forEach((feature) => {
    ...
    const territory = Object.keys(territories).find((territoryName) =>
        territories[territoryName].includes(stateName)
    );
...
const groupedBySalesRep: { [salesRep: string]: { territories: string[]; contact: string; phone: string } } = {};
...
svg.append("g")
    ...
    .on("mouseover", (event: MouseEvent, d: GeoJsonFeature) => {
        ...
        if (salesRep && groupedBySalesRep[salesRep]) {
            ...
            tooltip.html(
                `<strong>${stateAbbreviation}</strong><br>
                 <strong>${salesRep}</strong><br>`
